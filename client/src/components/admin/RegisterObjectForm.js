...
export default function RegisterObjectForm() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const uploadResponse = await axios.post('http://localhost:5000/api/objects/uploadObjectImage', formData, {
        headers: {
      ...
      const response = await axios.post('http://localhost:5000/api/objects', objectData);
  ...
  return (
    <div className="bg-green-500 py-24 sm:py-32 ">
      ...
      <h1 className="text-2xl font-bold text-center mb-4">Registrar objeto</h1>
